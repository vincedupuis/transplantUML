...
func main() {
	...
	stateMachineModel := convertScxmlToStateMachineModel(scxml)

	if *exportFileName != "" {
		exportStateMachineModelToJson(stateMachineModel, exportFileName)
	}

	tmplFile := readFile(tmplFileName)
	output := runTemplate(stateMachineModel, tmplFile)
	writeFile(outputFileName, output)
	fmt.Printf("Output file successfully written to: %s\n", *outputFileName)
}

func exportStateMachineModelToJson(stateMachine *internal.StateMachine, exportFileName *string) {
